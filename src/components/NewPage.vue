      ...
      for (let index=0; index<imgList.value.length; index++) {
        ...
        axios.post('http://127.0.0.1:8000/building/', form, {
          ...
          for (let j=0; j < selectedOptions.value.length; j++) {
            ...
            predictResult.value.push({date: (new Date()).toLocaleString(), image_name:_data['image_name'], content:_content[key], type:typeMapper[key]})
          }
        }).catch(error => {
          console.log(error)
          ElNotification({
    ...
    <div class="container">
        <el-col>
            <el-row :gutter="20" justify="center">
                <el-col :md="10" :sm="24" >
                    <div style="height: 46%;  margin-bottom: 20px;">
                ...
                <el-col :md="10" :sm="24" style="height: 500px;">
                    <div style="height: 100%;">
                        <el-card class="example-card">
                            ...
                            <el-carousel v-if="picShowValue == '示例图片'" height="380px" indicator-position="none">
                                <el-carousel-item v-for="img in sampleImageList">
                                    <el-image style="width: 56%;" :src="img.url" fit="cover" />
                                    <p>{{ img.desc }}</p>
                                </el-carousel-item>
                            </el-carousel>
                            <div v-else>
                                <el-empty v-if="imgList.length == 0" description="暂无图片" />
                                <el-table v-else :data="cachedImages" :show-header="false">
            ...
            <el-row justify="center">
                <el-col :span="20">
                    <el-card class="result-card">
                        ...
                        <el-table v-else :data="predictResult" :row-class-name="tableRowClassName" height="300px">
                            <el-table-column type="index" label="序号" width="80" />
                            <el-table-column prop="type" label="任务类型" width="120" sortable />
                            <el-table-column prop="date" label="识别日期" width="180" sortable />
